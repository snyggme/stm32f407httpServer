...
void http_server_serve(struct netconn *conn) 
{
  ...
  if (recv_err == ERR_OK)
  {
    if (netconn_err(conn) == ERR_OK) 
    {
      ...
      {
        /* Check if request to get style.css */ 
        if (strncmp((char const *)buf,"GET /css/style.css",18)==0)
        {
          file = fs_open("/css/style.css"); 
          netconn_write(conn, (const unsigned char*)(file->data), (size_t)file->len, NETCONN_NOCOPY);
          fs_close(file);
        }   
        /* Check if request to get first-attr.js */
        else if (strncmp((char const *)buf,"GET /js/first-attr.js",21)==0)
        {
          file = fs_open("/js/first-attr.js"); 
          netconn_write(conn, (const unsigned char*)(file->data), (size_t)file->len, NETCONN_NOCOPY);
          fs_close(file);
        }
        ...
        else if((strncmp(buf, "GET /index-attr.html", 20) == 0)||(strncmp(buf, "GET / ", 6) == 0)) 
        {
          /* Load main page */
          file = fs_open("/index-attr.html"); 
          netconn_write(conn, (const unsigned char*)(file->data), (size_t)file->len, NETCONN_NOCOPY);
          fs_close(file);
        }
        else 
...
{ 
  ...
  u16_t len;
  err_t recv_err;
	const char welcome[] = "Welcome to STM32F4 tcp server";
